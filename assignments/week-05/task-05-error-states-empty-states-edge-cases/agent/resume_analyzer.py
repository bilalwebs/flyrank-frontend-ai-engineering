...
def _parse_json_from_text(text: str) -> AnalysisResult:
    ...
    Raises:
        InvalidResponseError: If the text is empty.
        InvalidJSONError: If no valid JSON can be extracted.
    """
...
def _check_model_refusal(raw_text: str) -> None:
    """Detect if the model refused to perform the analysis."""
    refusal_signals = [
        "i cannot",
        "i can't",
        "i am unable",
...
async def analyze_resume(resume_path: str, job_description: str) -> AnalysisResult:
    ...
    Raises:
        AppError: On any validation, provider, or analysis error.
    """
    if not resume_path or not resume_path.strip():
        raise AnalysisError("Resume path cannot be empty.")
    ...
    logger.info(
        "Analysis started for resume: %s (model: %s, provider: %s, structured: %s)",
        resume_path, model_name, provider_name, use_structured,
    )
    ...
    try:
        ...
        elapsed = time.time() - start_time
        log_error(logger, exc, provider=provider_name, context="agent_analysis")
        classified = classify_error(exc, provider=provider_name)
        logger.error("Analysis failed after %.2fs: %s", elapsed, classified.user_message)
