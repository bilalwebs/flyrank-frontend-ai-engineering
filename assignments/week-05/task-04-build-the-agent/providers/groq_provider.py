...
class GroqProvider(ModelProvider):
    """Provides Groq models via the Chat Completions API.

    Groq exposes an OpenAI-compatible endpoint, so we use AsyncOpenAI
    with a custom base_url and the Chat Completions model class.
    """

    def __init__(self, api_key: str, model: str) -> None:
        self._model = model
        self._client = AsyncOpenAI(base_url=GROQ_BASE_URL, api_key=api_key)

    def get_model(self, model_name: str | None = None) -> Model:
        """Return a Chat Completions model backed by Groq.
